Code for the paper "Learning a Fast Mixing Exogenous Block MDP using a Single Trajectory" by Anonymous Authors.
...
Note that this will try to run all experiments simultanously; if running locally on a desktop, one may need to run the experiments sequentially to conserve system memory.
